...
void b_padArrUint8(const emxArray_uint8_T *Ain, unsigned char B[256])
{
  ...
  d_eml_find(b_A_data, A_size, (int *)&lastRow_data);
  b_A_size[0] = 1;
  b_A_size[1] = A->size[1];
  lastCol_data = A->size[1];
  for (i = 0; i < lastCol_data; i++) {
    b_A_data[i] = (A_data[A->size[0] * i] != 0);
  }
  e_eml_find(b_A_data, b_A_size, (int *)&lastCol_data, lastCol_size);
  for (i = 0; i < lastCol_data; i++) {
    for (A_size = 0; A_size < lastRow_data; A_size++) {
      A_data[A_size + lastRow_data * i] = A_data[A_size + A->size[0] * i];
    }
  }
  i = A->size[0] * A->size[1];
  ...
  for (i = 0; i < lastCol_data; i++) {
    for (A_size = 0; A_size < lastRow_data; A_size++) {
      B[A_size + (i << 4)] = A_data[A_size + lastRow_data * i];
    }
  }
  emxFree_uint8_T(&A);
}
                   ...
                   short N2, emxArray_uint8_T *B)
{
  ...
  for (i = 0; i < lastCol_data; i++) {
    for (A_size = 0; A_size < lastRow_data; A_size++) {
      B_data[A_size + B->size[0] * i] = A_data[A_size + lastRow_data * i];
    }
  }
  emxFree_uint8_T(&A);
}
